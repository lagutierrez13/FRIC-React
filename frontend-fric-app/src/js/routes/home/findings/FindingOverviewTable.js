...
class FindingOverviewTable extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    axios
      .get("http://localhost:4000/findingoverviewtable/get")
      .then((response) => {
  ...
  onSubmit(e) {
    ...
    const newFindingOverviewTable = {
      required: this.state.required,
      values: this.state.values,
    };

    const newHistory = {
      action: "Finding overview table changed",
      analyst: "",
    };

    axios
      .post(
        "http://localhost:4000/findingoverviewtable/new",
        newFindingOverviewTable
      )
  ...
  onChangeValueToAdd = (finding) => {
    this.setState({ value_to_add: finding.target.value });
  };
