...
class EffectivenessRatingTable extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    axios
      .get("http://localhost:4000/configuration/get/effectivenessrating")
      .then((response) => {
  ...
  onSubmit(e) {
    ...
    const newConfiguration = {
      tablename: "effectivenessrating",
      required: this.state.required,
      values: this.state.values,
    };

    const newHistory = {
      action: "Efectiveness Rating table changed",
      analyst: "",
    };
    ...
    return (
      <Col md={6} sm={6} xs={12}>
        <XPanel visible={panelVisible} onHide={onHide}>
          <XPanel.Title title="Effectiveness Rating Table">
...
export default EffectivenessRatingTable;
