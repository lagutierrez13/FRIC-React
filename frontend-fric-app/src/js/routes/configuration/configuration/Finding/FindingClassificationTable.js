...
class FindingClassificationTable extends Component {
  ...
  componentDidMount() {
    axios
      .get("http://localhost:4000/configuration/get/findingclassification")
      .then((response) => {
  ...
  onSubmit(e) {
    ...
    const newConfiguration = {
      tablename: "findingclassifiaction",
      required: this.state.required,
      values: this.state.values,
    };

    const newHistory = {
      action: "Finding classification table changed",
      analyst: "",
    };
    ...
    return (
      <Col md={6} sm={6} xs={12}>
        <XPanel visible={panelVisible} onHide={onHide}>
          <XPanel.Title title="Finding Classification Table">
            ...
            <form onSubmit={this.onSubmit}>
              <div className="form-check form-check-inline">
                <input
                  ...
                  name="required"
                  id="findingClassificationRequired"
