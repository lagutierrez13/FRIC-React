...
class FindingImpactLevelTable extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    axios
      .get("http://localhost:4000/configuration/get/findingimpactlevel")
      .then((response) => {
  ...
  onSubmit(e) {
    ...
    const newConfiguration = {
      tablename: "findingimpactlevel",
      required: this.state.required,
      values: this.state.values,
    };

    const newHistory = {
      action: "Finding impact level table changed",
      analyst: "",
    };
    ...
    return (
      <Col md={6} sm={6} xs={12}>
        <XPanel visible={panelVisible} onHide={onHide}>
          <XPanel.Title title="Finding Impact Level Table">
...
export default FindingImpactLevelTable;
